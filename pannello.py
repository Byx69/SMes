...
connessione = sqlite3.connect(parametri.database)
cursore = connessione.cursor()
cursore.execute("""SELECT id FROM impianti_pannello WHERE codice=?""", (parametri.codice_pannello,))
pannello_id = cursore.fetchone()[0]


def registra(evento):
    cursore = connessione.cursor()
    cursore.execute("""SELECT impianto_id, operatore_id, ordine_id FROM impianti_pannello WHERE id=?""", (pannello_id,))
...
def acceso():
    cursore = connessione.cursor()
    cursore.execute("""UPDATE impianti_pannello SET acceso=1 WHERE id=?""", (pannello_id,))
    connessione.commit()
    registra('ON')

def spento():
    cursore = connessione.cursor()
    cursore.execute("""UPDATE impianti_pannello SET acceso=0 WHERE id=?""", (pannello_id,))
    connessione.commit()
    registra('OFF')

def allarmescattato():
    cursore = connessione.cursor()
    cursore.execute("""UPDATE impianti_pannello SET allarme=1 WHERE id=?""", (pannello_id,))
    connessione.commit()
    registra('ALS')

def allarmerientrato():
    cursore = connessione.cursor()
    cursore.execute("""UPDATE impianti_pannello SET allarme=0 WHERE id=?""", (pannello_id,))
    connessione.commit()
    registra('ALR')
...
def operatoredisinserito(tessera):
    ...
    cursore.execute("""SELECT id FROM operatori_operatore WHERE tessera=?""", (tessera,))
    operatore_id = cursore.fetchone()[0]
    cursore.execute("""UPDATE impianti_pannello SET operatore_id=NULL WHERE id=?""", (pannello_id,))
    connessione.commit()

def apriordine(ordine_id):
    cursore = connessione.cursor()
    cursore.execute("""UPDATE impianti_pannello SET ordine_id=? WHERE id=?""", (ordine_id, pannello_id,))
    connessione.commit()
    cursore.execute("""UPDATE ordini_ordine SET data_apertura=? WHERE id=?""", (datetime.datetime.now(), ordine_id,))
    connessione.commit()    
    registra('ORA')  
